...
try {
    ...
    $error = "Route tables not found. Please run database.sql first.";
}
    ...
    if ($_SERVER["REQUEST_METHOD"] === "POST") {
        ...
        if ($action === "create_route") {
            ...
            if (empty($name)) {
                ...
                try {
                    $stmt = $pdo->prepare(
                        "INSERT INTO route_definitions (name) VALUES (?)",
                    );
                    $stmt->execute([$name]);
                    $success =
                        'Route "' .
                        htmlspecialchars($name) .
                        '" created. Add stops below.';
                } catch (PDOException $e) {
            ...
            if (!$route_id || $stop_name === "" || $lat === "" || $lng === "") {
                $error =
                    "Route, stop name, latitude and longitude are required.";
            } else {
                try {
                    $stmt = $pdo->prepare(
                    ...
                    $success =
                        'Stop "' . htmlspecialchars($stop_name) . '" added.';
                } catch (PDOException $e) {
            ...
            if ($route_id) {
                $stmt = $pdo->prepare(
                    "DELETE FROM route_stops WHERE route_definition_id = ?",
                );
                $stmt->execute([$route_id]);
                $stmt = $pdo->prepare(
                    "DELETE FROM route_definitions WHERE id = ?",
                );
                $stmt->execute([$route_id]);
            ...
            if ($route_id && !empty($order)) {
                try {
                    $stmt = $pdo->prepare(
                        "UPDATE route_stops SET stop_order = ? WHERE id = ? AND route_definition_id = ?",
                    );
            ...
            if ($route_id) {
                try {
                    ...
                    $success =
                        "Route saved. It is now in your list and visible to commuters.";
                } catch (PDOException $e) {
...
<body class="bg-[var(--transit-foundation)]">
    <div class="min-h-screen">
        <aside id="adminSidebar" class="fixed top-4 inset-x-4 md:inset-x-auto md:left-4 md:w-64 md:h-[calc(100vh-2rem)] glass-sidebar text-white flex flex-col z-30 rounded-2xl shadow-2xl">
            <div class="px-4 py-4 sm:p-6 flex-shrink-0 border-b border-[#475569] md:border-b-0">
                ...
                <nav id="adminNavLinks" class="space-y-1 md:space-y-2 text-sm sm:text-base hidden md:block">
                    <a href="admin_dashboard.php" class="flex items-center px-4 py-3 hover:bg-[#475569] rounded-lg transition duration-150 group">
                        <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"></path></svg>
                        Dashboard
                    </a>
                    <a href="admin_reports.php" class="flex items-center px-4 py-3 hover:bg-[#475569] rounded-lg transition duration-150 group">
                        <svg class="w-5 h-5 mr-3 group-hover:text-[#fbbf24]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17v-6a2 2 0 012-2h6m-4-4l4 4-4 4"></path></svg>
                        Reports
                    </a>
                    <a href="admin_trust_management.php" class="flex items-center px-4 py-3 hover:bg-[#475569] rounded-lg transition duration-150 group">
                        <svg class="w-5 h-5 mr-3 group-hover:text-[#fbbf24]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                        Trust Management
                    </a>
                    <a href="route_status.php" class="flex items-center px-4 py-3 hover:bg-[#475569] rounded-lg transition duration-150 group">
                        <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 20l-5.447-2.724A1 1 0 013 16.382V5.618a1 1 0 011.447-.894L9 7m0 13l6-3m-6 3V7m6 10l4.553 2.276A1 1 0 0021 18.382V7.618a1 1 0 00-.553-.894L15 4m0 13V4m0 0L9 7"></path></svg>
                        Route Status
                    </a>
                    <a href="manage_routes.php" class="flex items-center px-4 py-3 bg-[#fbbf24] text-[#1e3a8a] rounded-lg hover:bg-[#f59e0b] transition duration-150 shadow-lg">
                        <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 20l-5.447-2.724A1 1 0 013 16.382V5.618a1 1 0 011.447-.894L9 7m0 13l6-3m-6 3V7m6 10l4.553 2.276A1 1 0 0021 18.382V7.618a1 1 0 00-.553-.894L15 4m0 13V4m0 0L9 7"></path></svg>
                        Manage Routes
                    </a>
                    <a href="heatmap.php" class="flex items-center px-4 py-3 hover:bg-[#475569] rounded-lg transition duration-150 group">
                        <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"></path></svg>
                        Crowdsourcing Heatmap
                    </a>
                    <a href="user_management.php" class="flex items-center px-4 py-3 hover:bg-[#475569] rounded-lg transition duration-150 group">
        ...
        <main class="w-full md:ml-72 pt-24 md:pt-0 overflow-y-auto">
            <div class="p-4 sm:p-6 lg:p-8">
                ...
                <?php if ($error): ?>
                    <div class="mt-4 bg-red-100 border-l-4 border-red-500 text-red-700 px-4 py-3 rounded"><?php echo htmlspecialchars(
                        $error,
                    ); ?></div>
                <?php endif; ?>
                <?php if ($success): ?>
                    <div class="mt-4 bg-green-100 border-l-4 border-green-500 text-green-700 px-4 py-3 rounded"><?php echo htmlspecialchars(
                        $success,
                    ); ?></div>
                <?php endif; ?>
    ...
    "id"
                            ...
                            <div class="px-6 py-4 bg-white/30 border-b border-white/20 flex flex-wrap justify-between items-center gap-2">
                                ...
                                <div class="flex items-center gap-2">
                                    <form method="POST" id="save-route-form-<?php echo (int) $route[
                                        "id"
                                    ]; ?>" class="inline">
                            ...
                            <div class="p-6 flex flex-col lg:flex-row gap-6">
                                <div class="lg:w-1/2 space-y-4">
                                    ...
                                    <ul id="stops-list-<?php echo (int) $route[
                                        "id"
                                    ]; ?>" class="space-y-1 text-sm text-gray-700" data-route-id="<?php echo (int) $route[
    "id"
]; ?>">
                                        <?php foreach (
                                            $route["stops"]
                                            ...
                                            <li draggable="true" data-stop-id="<?php echo (int) $s[
                                                "id"
                                            ]; ?>" class="flex items-center gap-2 p-2 rounded border border-gray-200 bg-gray-50 hover:bg-gray-100 cursor-move select-none">
                                                <span class="text-gray-400 cursor-move" title="Drag to reorder">⋮⋮</span>
                                                <span class="flex-1"><?php echo htmlspecialchars(
                                                    $s["stop_name"],
                                                ); ?></span>
                                                <form method="POST" onsubmit="return confirm('Remove this stop?');" class="inline">
                                                    <input type="hidden" name="action" value="delete_stop">
                                                    <input type="hidden" name="stop_id" value="<?php echo (int) $s[
                                                        "id"
                                                    ]; ?>">
                                    ...
                                    <?php endif; ?>
                                    <form method="POST" id="add-stop-form-<?php echo (int) $route[
                                        "id"
                                        ...
                                        <div>
                                            ...
                                            <div class="flex gap-2 mb-1">
                                                <input type="text" id="place-search-<?php echo (int) $route[
                                                    "id"
                                                ]; ?>" placeholder="Search Philippines locations (autocomplete enabled)" class="flex-1 px-3 py-2 border border-gray-300 rounded-md text-sm">
                                                <button type="button" id="place-search-btn-<?php echo (int) $route[
                                                    "id"
                                                ]; ?>" class="bg-blue-600 text-white px-3 py-2 rounded-md text-sm font-medium hover:bg-blue-700">Search</button>
                                            </div>
                                            <div id="place-results-<?php echo (int) $route[
                                                "id"
                                            ]; ?>" class="hidden mt-1 max-h-48 overflow-y-auto border border-gray-200 rounded bg-white shadow text-sm"></div>
                                            ...
                                            <p id="pin-status-<?php echo (int) $route[
                                                "id"
                                            ]; ?>" class="text-xs text-gray-500 mt-0.5">No location set.</p>
                                            <input type="hidden" name="latitude" id="lat-<?php echo (int) $route[
                                                "id"
                                            ]; ?>" required>
                                            <input type="hidden" name="longitude" id="lng-<?php echo (int) $route[
                                                "id"
                                            ]; ?>" required>
                                        </div>
                                        <input type="hidden" name="stop_order" value="<?php echo count(
                                            $route["stops"],
                                        ); ?>">
                                    ...
                                    <script>
                                    document.getElementById('add-stop-form-<?php echo (int) $route[
                                        "id"
                                    ]; ?>').addEventListener('submit', function(e) {
                                        var lat = document.getElementById('lat-<?php echo (int) $route[
                                            "id"
                                        ]; ?>').value;
                                        var lng = document.getElementById('lng-<?php echo (int) $route[
                                            "id"
                                        ]; ?>').value;
                                        if (!lat || !lng) {
                                            e.preventDefault();
                                    ...
                                    document.getElementById('save-route-form-<?php echo (int) $route[
                                        "id"
                                    ]; ?>').addEventListener('submit', function() {
                                        var list = document.getElementById('stops-list-<?php echo (int) $route[
                                            "id"
                                        ]; ?>');
                                        var form = document.getElementById('save-route-form-<?php echo (int) $route[
                                            "id"
                                        ]; ?>');
                                ...
                                <div class="lg:w-1/2 h-64 rounded-lg overflow-hidden border border-gray-200" id="map-route-<?php echo (int) $route[
                                    "id"
                                ]; ?>"></div>
                            </div>
                        </div>
    ...
    <script>
        ...
        function displaySearchResults(data, routeId, resultsEl, setStopFn) {
            ...
            data.forEach(function (item, index) {
                ...
                let icon = '<svg class="h-4 w-4 mr-2 text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 21v-4m0 0V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path></svg>';
